...
<div class="flex justify-center items-center min-h-[70vh] px-4">
    <div class="bg-card/80 backdrop-blur-xl border border-[#2d3342] p-8 md:p-10 rounded-3xl w-full max-w-md shadow-2xl fade-in relative overflow-hidden">
        {{-- Hiasan Interaktif --}}
        <div class="absolute -top-10 -left-10 w-32 h-32 bg-orange/20 rounded-full blur-2xl pointer-events-none"></div>

        <h2 class="text-3xl font-black text-white mb-8 text-center tracking-tight relative z-10">Welcome Back</h2>
        
        {{-- Teks Dinamis: Pesan Error --}}
        @if ($errors->any())
            <div class="bg-red-500/10 border border-red-500/50 text-red-400 p-4 rounded-xl mb-8 text-sm flex items-center gap-3 font-medium animate-pulse">
                <i class="fa-solid fa-circle-exclamation text-lg"></i> {{ $errors->first() }}
            </div>
        @endif

        {{-- Form Login Dipertahankan 100% --}}
        <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-5 relative z-10">
            @csrf
            <div class="group">
        ...
        <p class="mt-8 text-center text-sm text-gray-400 font-medium relative z-10">
            Don't have an account? <a href="{{ route('register') }}" class="text-orange hover:text-[#ea580c] transition-colors border-b-2 border-orange/30 hover:border-orange pb-0.5">Register now</a>
        </p>
    </div>
</div>
@endsection